...
````
##Ниже описан порядок настройки Raspberry Pi (rpi)
...
````
  ...
  Пароль: raspberry
````
Далее необходимо настроить ip адрес rpi или
...
````
  ssh rpi
````
В случае необходимости, для подключения rpi
к сети интернет необходимо выполнить туннель
...
````
Дальнейшая инструкция выполняется из под root пользователя
...
````
Для выбора интерфейса для dhcp сервера,
